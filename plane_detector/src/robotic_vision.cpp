...
void cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob) {
	...
	*temp_cloud = *cloud_blob;
	...
	if(!new_data)	{
		...
		//nclouds++; //USED IF CUMULATIVE AVERAGE IS CHOSEN
		...
		for(int j=0;j<mean_cloud->height;j++){
			for(int i=0;i<mean_cloud->width;i++){
				...
				if(!origin_eliminated && mean_cloud->at(i,j).x==0.0f && mean_cloud->at(i,j).y==0.0f) { 
					mean_cloud->at(i,j).x =NAN;
					mean_cloud->at(i,j).y =NAN;
					mean_cloud->at(i,j).z =NAN;
					origin_eliminated=true;
				}
				//POINTCLOUD AVERAGING UPDATE
				if(new_cloud->at(i,j).x!=0.0f && new_cloud->at(i,j).y!=0.0f) {
					//EXPONENTIAL MOVING AVERAGE (CAN BE MADE MORE RESPONSIVE BY LOWERING PARAMETER "ema_coeff")
					mean_cloud->at(i,j).x = ema_coeff*mean_cloud->at(i,j).x + (1-ema_coeff)*new_cloud->at(i,j).x;
					mean_cloud->at(i,j).y = ema_coeff*mean_cloud->at(i,j).y + (1-ema_coeff)*new_cloud->at(i,j).y;
					mean_cloud->at(i,j).z = ema_coeff*mean_cloud->at(i,j).z + (1-ema_coeff)*new_cloud->at(i,j).z;
					/* CUMULATIVE AVERAGE (MORE STABLE BUT ALMOST UNRESPONSIVE TO DYNAMIC ENVIRONMENTS)
					mean_cloud->at(i,j).x = mean_cloud->at(i,j).x + ((new_cloud->at(i,j).x - mean_cloud->at(i,j).x)/nclouds);
					mean_cloud->at(i,j).y = mean_cloud->at(i,j).y + ((new_cloud->at(i,j).y - mean_cloud->at(i,j).y)/nclouds);
					mean_cloud->at(i,j).z = mean_cloud->at(i,j).z + ((new_cloud->at(i,j).z - mean_cloud->at(i,j).z)/nclouds);
					*/
	    			}
	    	}
	...
	while(ros::ok()){
		...
		if(new_data){
			...
			for (std::vector<int>::iterator it = remaining->begin(); it != remaining->end(); ++it) {
	  		...
	  		//ALIGN THE POINTCLOUD WITH RVIZ FRAME(X-AXIS)------------------------------------------------------------
	    		...
	    		for(int i=0;i<ground_cloud->points.size();i++){
	    			if(ground_cloud->points[i].x>0)
	    				if(ground_cloud->points[i].y < ma_y){
	  		...
	  		n_align.normalize();
	  		rotate_point_cloud_plane_v2(input_cloud,n_align,n_x);
			...
			rtrack_obs_cloud->height=1;
	  		//Identify obstacle points near tracks
	    		if(swing_leg==-1){ //both tracks have to be processed
	    			for(int i=0;i<obs_cloud->points.size();i++){
	    				...
	    				if(condition_3){
	    					if(condition_1) { 
	    						...
	    						if(obs_cloud->points[i].z>0.2 && obs_cloud->points[i].y<high_obs_bound_y){
	    		...
	    		else if(swing_leg==0) { //only left track has to be processed
	    				for(int i=0;i<obs_cloud->points.size();i++){
		    				condition_1 = obs_cloud->points[i].x>-bound_1 && obs_cloud->points[i].x<-bound_2;
		    				condition_3 = obs_cloud->points[i].y < bound_3 && obs_cloud->points[i].y>=0; //neglect obstacles behind the camera so that feet are not considered;
	    		...
	    		else { //only right track has to be processed
	    				for(int i=0;i<obs_cloud->points.size();i++){
		    				condition_2 = obs_cloud->points[i].x<bound_1 && obs_cloud->points[i].x>bound_2;
		    				condition_3 = obs_cloud->points[i].y < bound_3 && obs_cloud->points[i].y>=0; //neglect obstacles behind the camera so that feet are not considered;
	  		...
	  		float mean = 0.3f; //mean of optimal step length
	    				...
	    				if(condition_3){
	    					if(condition_1) { //left track
	    						...
	    						for(int j=0;j<ltrack_obs_cloud->points.size();j++){
	    							if(ltrack_obs_cloud->points[j].y<high_obs_bound_y){
	    					...
	    					if(condition_2) { //right track
	    						...
	    						for(int j=0;j<rtrack_obs_cloud->points.size();j++){
	    							if(rtrack_obs_cloud->points[j].y<high_obs_bound_y){
			...
			pcl::PCLPointCloud2 outcloud;
  			...
  			pub6.publish(msg);
  			msg.data = max_score_win_y_pos[next_swing_leg];
  			pub7.publish(msg);
